...
function loadPlaylistCategories({ message: name, playlists: { items } }) {
  ...
  for (let { description, images, id, name } of items) {
    ...
    mainContainer.className =
      "overflow-hidden group relative flex cursor-pointer flex-col items-center justify-between gap-2 rounded-md p-2 hover:bg-gray";
...
function loadPlaylistTracks({ tracks }) {
  ...
  for (let trackItem of tracks.items.filter((item) => item.track.preview_url)) {
    ...
    track.id = id;
    track.className =
      "cursor-pointer hover:bg-gray track p-1 grid grid-cols-[50px_1fr_1fr_50px] gap-4 items-center justify-items-start gap-4 rounded-md";
...
async function fillContentForPlaylist(playlistId) {
  const playListInfo = await fetchRequest(
    `${END_POINTS.PLAYLIST}/${playlistId}`,
  );
...
function updateIconsForPlayMode(selectedTrackId) {
  ...
  const playButtonFromTracks = document.querySelector(
